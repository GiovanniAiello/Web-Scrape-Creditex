...
path_output_img = os.getcwd() + "/Output_image"
...
for year in years0:
    auctionIds = reqpars_y(urlyears1, year).find_all("span",
                                                     class_="standalonelink")  # import all the anchor that have "holdings.jsp" in their href
    ...
    for i in range(length):
        ...
        for idx, img_url in enumerate(soup.select(
                'img[src^="/information/affiliations/fixings/20"]')):  # add the name of the company whose CDS are auctioned before each table
            sc = requests.get(main_url.format(id=img_url['src']), stream=True)
            table_title = img_url.find_previous('h2').text
            output_img = path_output_img + "/" + str(year) + " " + ticker + " " + str(idx) + " " + str(table_title)[
                                                                                                    0:34] + img_url['src'][-4:]
            if sc.status_code == 200:
                with open(output_img,'wb') as f:
                    shutil.copyfileobj(sc.raw, f)
            img = Image.open(output_img).convert('RGB')
            img.save(path_output_img + "/" + str(year) + " " + ticker + " " + str(idx) + " " + str(table_title)[0:34]+".jpg")
            output_cvs = path_output_csv + "/" + str(year) + " " + ticker + " " + str(idx) + " " + str(table_title)[
                                                                                                0:34] + ".csv"
...
for year in years1:
    ...
    for i in range(length):
        ...
        for idx, table in enumerate(
            ...
            df.to_csv(path_output_csv + "/" + str(year) + " " + ticker + " " + str(idx) + " " + str(table_title)[
                                                                                                0:34] + ".csv",
                      index=False)
...
for i in range(length):
    ...
    print(auctionIds[i].text)
    for idx, table in enumerate(
            soup3.find_all('table')):  # add the name of the company whose CDS are auctioned before each table
        temp = pandas.read_html(str(table))  # print all the table
        df = pandas.DataFrame(temp[0])  # title = str(year)+ " " +auction_name+str(idx)+".csv"
        table_title = table.find_previous('h2').text
        df.to_csv(
            path_output_csv + "/" + str(year) + " " + ticker + " " + str(idx) + " " + str(table_title)[0:34] + ".csv",
            index=False)
...
for i in range(length):
    ...
    for idx, table in enumerate(soup2.find_all('table')):
        ...
        df = pandas.DataFrame(temp[0])  # title = str(year)+ " " +auction_name+str(idx)+".csv"
        table_title = table.find_previous('h2').text
        df.to_csv(path_output_csv + "/" + str(year) + " " + auction_name + " " + str(idx) + " " + str(table_title)[
                                                                                                  0:34] + ".csv",
                  index=False)

year = 2010
auctionIds = reqpars_y(urlyears3, year).select(
    'a[href^="/information/affiliations/fixings/20"]')  # import all the anchor that have "holdings.jsp" in their href
length = len(auctionIds)  # how many auction for each year
...
for year in years3:
    auctionIds = reqpars_y(urlyears3, year).select(
        'a[href^="holdings.jsp"]')  # import all the anchor that have "holdings.jsp" in their href
    length = len(auctionIds)  # how many auction for each year
    for i in range(length):
        id1 = auctionIds[i].get('href')  # now let's extract the link to each auction
        ticker = reqpars_id(urlid, id1).select(
            'a[href^="results.jsp"]')  # select the anchor that have results.jsp in href
