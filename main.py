...
for year in years1:
    ...
    for i in range(length):
        ...
        html2 = session.get(main_url.format(
            id=id1)).text  # this the auction page and we need to select the link the bring us to the results
        soup2 = BeautifulSoup(html2, "html.parser")  # parse the html
        auction_name = id1.split(str(year)+"/",1)[1][:-6]
        print(auction_name)
        for idx, table in enumerate(soup2.find_all('table')):  # add the name of the company whose CDS are auctioned before each table
            ...
            df.to_csv(os.getcwd() + "/" + str(year) + " " + auction_name + " " + str(idx) + " " + str(table_title)[0:34] + ".csv",index=False)
...
for i in range(length):
    ...
    html3 = session.get(main_url.format(id=id2)).text  # this the auction page and we need to select the link the bring us to the results
    soup3 = BeautifulSoup(html3, "html.parser")
    auction_name = id2.split(str(year)+"/",1)[1][:-10]
...
for i in range(length):
    ...
    html2 = session.get(url2009id.format(
        id=id2)).text  # this the auction page and we need to select the link the bring us to the results
...
for year in years3:
    ...
    for i in range(length):
        id1 = auctionId[i].get('href')  # now let's extract the link to each auction
        html2 = session.get(urlid.format(
            id=id1)).text  # this the auction page and we need to select the link the bring us to the results
        soup2 = BeautifulSoup(html2, "html.parser")  # parse the html
